%%
%
% Problem 4: Shape Analysis on MRI Images of Human Brain
%

% SHOW_FIGURES = true;

image_folder = '../data/brain/data/data/';
filenames = dir(fullfile(image_folder, '*.png'));
num_images = numel(filenames);

num_points = 32; % No. of points in each pointset
...
brain_pointsets_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
    ...
    title('Initial Shape-set - Brain (Translated and Scaled)');
end
...
brain_meanshape_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
...
title('Mean Shape - Brain');

%% Covariance Matrix

[V, L] = computeModesOfVariation(shapes, mean_shape);

z_mean = mean_shape(:);
brain_variance_fig = figure('visible', SHOW_FIGURES);
variances = diag(L);
plot(variances(1:3));
title('Eigenvalues plot - Brain');
...
brain_mode1_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
    scatter(aligned_shapes(1, :, i), aligned_shapes(2, :, i), 1, rand(1, 3)); 
end
...
title('First mode of Variation - Brain');
legend([p1, p2, p3]);
...
brain_mode2_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
    scatter(aligned_shapes(1, :, i), aligned_shapes(2, :, i), 1, rand(1, 3)); 
end
...
p2 = plot([z1(1:2:size(z1, 1), 1)' z1(1, 1)], [z1(2:2:size(z1, 1), 1)' z1(2, 1)], 'DisplayName','+3 SD from mean');
p3 = plot([z2(1:2:size(z1, 1), 1)' z2(1, 1)], [z2(2:2:size(z2, 1), 1)' z2(2, 1)], 'DisplayName','-3 SD from mean');
title('Second mode of Variation - Brain');
legend([p1, p2, p3]);
...
brain_mode3_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
    scatter(aligned_shapes(1, :, i), aligned_shapes(2, :, i), 1, rand(1, 3)); 
end
...
p2 = plot([z1(1:2:size(z1, 1), 1)' z1(1, 1)], [z1(2:2:size(z1, 1), 1)' z1(2, 1)], 'DisplayName','+3 SD from mean');
p3 = plot([z2(1:2:size(z1, 1), 1)' z2(1, 1)], [z2(2:2:size(z2, 1), 1)' z2(2, 1)], 'DisplayName','-3 SD from mean');
title('Third mode of Variation - Brain');
legend([p1, p2, p3]);

%% Most Similar
brain_closesttomean_fig = figure('visible', SHOW_FIGURES);
min_dist = Inf;
for i = 1:num_images
...
title('Closest to Mean Shape - Brain');
...
brain_closesttominus3_fig = figure('visible', SHOW_FIGURES);
hold on
