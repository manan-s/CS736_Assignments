%%
%
% Problem 2: Shape Analysis on Human Hand Shapes
%
...
hand_pointsets_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
    ...
    title('Initial Shape-set - Hand (Raw)');
end
...
hand_meanshape_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
...
z_mean = mean_shape(:);
hand_variance_fig = figure('visible', SHOW_FIGURES);
variances = diag(L);
plot(variances(1:3));
title('Eigenvalues plot - Hand');
...
hand_mode1_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
    scatter(aligned_shapes(1, :, i), aligned_shapes(2, :, i), 1, rand(1, 3)); 
end
...
title('First mode of Variation - Hand');
legend([p1, p2, p3]);
...
hand_mode2_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
    scatter(aligned_shapes(1, :, i), aligned_shapes(2, :, i), 1, rand(1, 3)); 
end
...
p2 = plot([z1(1:2:size(z1, 1), 1)' z1(1, 1)], [z1(2:2:size(z1, 1), 1)' z1(2, 1)], 'DisplayName','+3 SD from mean');
p3 = plot([z2(1:2:size(z1, 1), 1)' z2(1, 1)], [z2(2:2:size(z2, 1), 1)' z2(2, 1)], 'DisplayName','-3 SD from mean');
title('Second mode of Variation - Hand');
legend([p1, p2, p3]);
...
hand_mode3_fig = figure('visible', SHOW_FIGURES);
hold on;
for i = 1:num_images
    scatter(aligned_shapes(1, :, i), aligned_shapes(2, :, i), 1, rand(1, 3)); 
end
...
p2 = plot([z1(1:2:size(z1, 1), 1)' z1(1, 1)], [z1(2:2:size(z1, 1), 1)' z1(2, 1)], 'DisplayName','+3 SD from mean');
p3 = plot([z2(1:2:size(z1, 1), 1)' z2(1, 1)], [z2(2:2:size(z2, 1), 1)' z2(2, 1)], 'DisplayName','-3 SD from mean');
title('Third mode of Variation - Hand');
legend([p1, p2, p3]);

%% Most Similar
hand_closesttomean_fig = figure('visible', SHOW_FIGURES);
min_dist = Inf;
for i = 1:num_images
...
title('Closest to Mean Shape - Hand');
...
for i = 1:num_images
    z = shapes(:, :, i);
    dist = sqrt(sum((z(:) - minus3_shape(:)).^2));
    if dist < min_dist
        min_dist = dist;
        image_index = i;
    end
...
hand_closesttominus3_fig = figure('visible', SHOW_FIGURES);
hold on
